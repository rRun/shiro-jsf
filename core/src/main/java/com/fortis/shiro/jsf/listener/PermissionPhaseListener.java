...
public class PermissionPhaseListener implements PhaseListener {
  ...
  /**
   * 构建方法.
   * @param loginUrl 登陆的页面URL
   * @param successUrl 登陆成功的页面URL
   * @param errorUrl 登陆失败的页面URL
   */
  ...
  public void afterPhase(PhaseEvent event) {
    ...
    if (event.getPhaseId() == PhaseId.RESTORE_VIEW) {
      //TODO: 鉴权代码
      String lastViewId = (String) SecurityUtils.getSubject()
          .getSession().getAttribute(lastViewIdKey);
      if (lastViewId == null) {
        SecurityUtils.getSubject().getSession()
            .setAttribute(lastViewIdKey,viewId == null ? loginUrl : viewId);
      }
      //如果还未认证
      ...
      Map<String,Integer> rules = new HashedMap();
      rules.put("/systemManager/systemMenu/",0);
